...
- `downloads: [<fichier>, …]` (dans `track.yaml`) réserve des fichiers de `DOWNLOADS_DIR` (servis par `/telechargements/<fichier>`) à ceux qui ont accès à tout le parcours : un compte qui ne l'a pas reçoit une 403, et le fichier d'un parcours en préparation n'existe pas pour les autres (404). Un fichier qu'aucun parcours ne déclare reste ouvert à tout compte connecté. Un moteur antérieur à la 2.1 ignore la clé, et sert alors le fichier à tout compte connecté.
